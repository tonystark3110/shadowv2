...
def spp(net,channel=64,scope='g_pool'):
    ...
    net = tf.concat([
      tf.image.resize(net_2,(tf.shape(input=net)[1],tf.shape(input=net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      tf.image.resize(net_8,(tf.shape(input=net)[1],tf.shape(input=net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      tf.image.resize(net_16,(tf.shape(input=net)[1],tf.shape(input=net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      tf.image.resize(net_32,(tf.shape(input=net)[1],tf.shape(input=net)[2]), method=tf.image.ResizeMethod.BILINEAR),
      net],axis=3)
...
def conv(batch_input, out_channels, stride):
    with tf.compat.v1.variable_scope("conv"):
        ...
        conv = tf.nn.conv2d(input=padded_input, filters=filter, strides=[1, stride, stride, 1], padding="VALID")
        return conv
...
def build_shadow_generator(input,channel=64):
    c0 = tf.pad(tensor=input, paddings=[[0, 0], [3, 3], [3, 3], [0, 0]], mode="REFLECT")
